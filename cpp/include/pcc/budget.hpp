...
//
// 生の点に Voronoi 極を当てる方法は使えなかった（標本密度に比例して収束しない）。
// 中軸はノイズに不安定で、測定ノイズが表面のすぐ近くに作る偽の枝を測ってしまう。
// 対策は「中軸を計算する前に平滑化する」こと。正則ボクセル格子で符号付き距離を
// 作り、零交差から曲面を取り出してから shrinking ball を回す。
#pragma once
#include <cstdint>
#include <vector>
#include <string>
