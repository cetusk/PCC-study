...
int main(int argc, char** argv) {
    ...
    if (cmd == "residue") {
        // 正規化を適用したあと、各フィールドを 1 つずつ定数で潰して
        // 実測の差分を取る。既存コーデックが「そのフィールドに実際に
        // 使っているビット数」が分かる（孤立コストではなく実測の内訳）。
        Plan plan = analyze(pc, true);
        std::vector<std::string> keep;
        std::map<std::string, std::vector<int64_t>> ext;
        apply_plan(pc, plan, keep, ext);
    ...
    // 解析。残差符号化は「入れた方が本当に小さくなるか」を実測で決める。
    // エントロピーの閾値だけで採用すると、既存コーデックが既に回収済みの
    // フィールドを外部に出して逆に太る（実測: autzen で −0.2%、plane で −1.2%）。
    double tan = now();
    ...
    if (spatial) {
        ...
        // 単体の差分符号長を代理指標にすると判断を誤る（実測で 2.3 bit/点）。
