...
| 文書 | 札 | 内容 |
|---|---|---|
| `paper/pcc.pdf` | 現行（一部未再測定） | 正規化レイヤーの論文。自前の符号器の表（`tab:alone`）と本文は 2026-09-23 の改善ループ後の値。§e2e（6.3% / 22.3%、295.2 MB）は LASzip を中に置く構成で、自前の符号器の改良は効かないが、同日の正規化の直し（線形関係の検出・負の値の粗い格子）の影響は測り直していない |
