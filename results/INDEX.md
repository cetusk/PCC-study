...
| 文書 | 札 | 内容 |
|---|---|---|
| `paper/pcc.pdf` | 現行 | 正規化レイヤーの論文。自前の符号器の表（`tab:alone`）と本文は 2026-09-23 の改善ループ後の値。§e2e（旧い経路 `pccnorm combine`）は 2026-09-23 に 200 万点で測り直し、「G-PCC の幾何 + 本手法」32.889 bpp がビット単位で再現した。800 万点の列とタイル全体（440 → 295.2 MB）は測り直していない |
