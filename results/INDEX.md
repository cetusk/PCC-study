...
| 文書 | 札 | 内容 |
|---|---|---|
| `paper/pcc.pdf` | 現行 | 正規化レイヤーの論文。自前の符号器の表（`tab:alone`）と本文は 2026-09-23 の改善ループ後の値。§e2e・表 `tab:e2e`（11 ファイル）・`tab:grid` を 2026-09-23 に全部測り直した（`data/work/grid/e2e_v19/`）。combine の 200 万点 32.889・800 万点 33.307 bpp はビット単位で再現、タイル全体は 295.2 → 295.0 MB |
