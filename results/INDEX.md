...
| 文書 | 札 | 内容 |
|---|---|---|
| `paper/pcc.pdf` | 現行 | 正規化レイヤーの論文。自前の符号器の表（`tab:alone`）と本文は 2026-09-23 の改善ループ後の値。§e2e（旧い経路 `pccnorm combine`）は 2026-09-23 に測り直し、200 万点 32.889 bpp・800 万点 33.307 bpp がビット単位で再現した。タイル全体は 295.2 → 295.0 MB（`pccnorm bench --spatial --full-search`、ビット完全） |
