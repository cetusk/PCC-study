...
最終更新: 2026-09-19（11 ファイル一括測定の完了まで）
...
| 走査モデル符号器 v1〜v4 | **完了**。v1 が 11 件中 10 件で最良。v2/v3/v4 は外す候補 |
| 11 ファイル一括測定 | **完了**（2026-09-19、所要 3 時間 34 分、全件で検証通過） |
| 「決して悪化しない」保証 | **未着手**。11 件中 3 件で LASzip に負ける（A-3・最優先） |
| 掃引の分割の改善 | **未着手**。勝敗を決めている量（B-1） |
| 可逆な多重解像度（mipmap / パッチ） | 未着手 |

一括測定の結果は [`results/scan_model_fitting.md`](results/scan_model_fitting.md) の
16〜18 節にある。走査モデルが採られたのは 11 件中 3 件で、
勝敗は当てはめではなく**走査線の切り出し**で決まっていた。
...
```bash
bash scripts/run_scan_batch.sh          # 直列 12 ジョブ、約 3 時間 34 分
