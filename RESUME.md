...
> **LAZ に 17/17**（0.89x）、符号化 43.9x・復号 8.1x・742 byte/点（`bench_2m_v21.log`）。
> 2026-09-23 に器の版を 4 に上げた（非可逆の極座標を自前の sin/cos で戻す。`results/defects.md` §7）。
> **版 3 の .pcc2 は読める**（極座標だけ libm で戻す）。**版 1・2 は読めない**ので作り直すこと。
