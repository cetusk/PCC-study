...
> 要点（2026-09-24）: 全列 対 LASzip **−16.3%（15/15、中央値。幅 −38.2〜−9.8%）**、幾何単体で
> **LAZ に 17/17**（0.87x）、符号化 47.1x・復号 9.9x・928 byte/点（`bench_2m_v24.log`）。
> 2026-09-23 に器の版を 4 に（非可逆の極座標を自前の sin/cos で戻す。`results/defects.md` §7）、09-24 に 5 に上げた
> （旗「類」。`results/losses.md` §34）。**版 3・4 の .pcc2 は読める**（版 3 は極座標だけ libm で戻す）。**版 1・2 は読めない**ので作り直すこと。
