...
| 符号化の速度 | **2026-09-21 に 99.3 倍 → 12.3 倍**（対 LASzip、全列、四分位 [8.9, 14.2]、幅 4.2〜17.2）。55〜64 節 |
| 復号の速度 | **4.3 倍**（同、四分位 [1.9, 5.4]）。復号は選ばれた 1 本しか通らない。62〜63 節 |
| 走査モデルの変種 | **2026-09-21 に v2/v3/v4 も既定で出す**（65 節）。幾何は G-PCC に 8/17 → 9/17、対 G-PCC 中央値 +0.3% → −3.7% |
| 標本での事前選別の代償 | **2026-09-21 に 0 に**（66 節）。`PCC_PRESELECT=0` の上限と全 15 件が一致（20 万点・100 万点とも）。100 万点で 5 件が 0.28〜1.01% 縮んだ |
