...
最終更新: 2026-09-20（順序感度の測定と、独立査読による訂正まで）
...
| 順序感度の測定 | **完了**（30 節）。G-PCC が帰無対照として機能することを確認 |
...
| 30 | 順序感度の本測定（12 データ × 8 条件 × 4 符号器） |

**同じ型の誤りを 3 度繰り返している。**（i）検証機構のない自作スクリプトで
数字を作る、（ii）不利なデータに頑健性検査を掛けない、（iii）後の節で撤回した
主張を元の節が断定したまま残す。数字を引用する前に 24・27・29 節を読むこと。
