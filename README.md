...
| | 中央値 | 四分位 |
|---|---|---|
| 符号化 | **15.0x**（開始時 99.3x） | [10.7, 31.5] |
| **復号** | **4.5x** | [2.3, 5.7] |

**符号化が復号の 4.0 倍かかるのは、候補を全部符号化して最短を採るためである**
（同じファイルで測った比の中央値）。復号は選ばれた 1 本しか通らない。

速度に効いたのは次の 9 つで、**最後の 1 つ以外はサイズを 1 bit も変えない**。
...
（AHN3 _20 100 万点・全 14 列の `enc`）

**「LAZ に匹敵する（1〜2 倍）」には届いていない。** 実装側で取れるものは
取り切っており、残りは候補を減らす（1.7〜4.5% のサイズ悪化を実測）か、
下位ビットを確率モデルなしで書く（候補として出しても 17 件で 1 度も
採られない）かで、**どちらもサイズを犠牲にする**。
