...
2. **鍵の縮退を制御して掃引する** — 順序感度の測定（30 節）で、走査モデルの順序不変性は鍵が完全に縮退すると失われると分かった。ただし「単射性が決めている」とは言えない（plane は鍵重複 82.4% でも +7.0% しか崩れない）。境界の形も示せていない（鍵重複 45〜82% にデータが無い）。1 ファイル内で鍵だけを壊して因果を直接試す（36・37 節で着手、100% の水準しか切れていない）
3. **gps_time の符号器** — AHN3 が負ける主因。ulp 格子を前提にしない表現が要る
