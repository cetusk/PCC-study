...
（`python/bench_all.py`、ログ `data/work/grid/bench_2m_v24.log`）。
...
幾何だけを取り出すと、**LAZ には 17/17 で勝つ**（比の中央値 0.87x）。
...
| | 中央値 | 四分位 |
|---|---|---|
| 符号化 | **47.1x**（開始時 99.3x） | [26.7, 74.7] |
| **復号** | **9.9x** | [2.3, 13.9] |

（LASzip との比は分母が揺れるので、費用は PCC2 の実時間で見る。2026-09-24 の S1〜S4 で PCC2 の符号化の秒数の合計は +28.2%、復号は +5.3%。）

メモリーは 1 点あたり中央値 **928 byte**（属性込み。S1〜S4 で 742 から増えた）。三者で最も重い（standing.md §6）。
...
- **検証一式** `python/verify_suite.py`（10 項目、単独で回す）: 回帰 63 件・15 件の往復と照合・
  スレッド数を変えた md5 一致・次元の被覆・`unpack --las` の生バイト比較・PLY・KITTI 108 frame・
  幾何の候補 58 本の強制・非可逆（極座標とデカルト格子）・命令セットの違う二値との一致（81/81）
- **器の版は 5**（版 3・4 も読める。版 5 で旗「類」を足した）。**版 4 以降の器の復号は libm の三角関数を呼ばない**（非可逆の極座標は四則演算と
...
| 車載 LiDAR の float32 は 1 mm 格子に乗る（可逆に整数化） | 無圧縮比 −83.3%（108 frame の中央値、強度込み。`kitti108_v24.log`）、座標だけなら走行全体で 16.901 bpp（−82.4%、`kitti_xyz_v22.log`）、整数 LAS にした LASzip 比 −6.6% | [forward_operator.md](results/forward_operator.md) §4（発見）・[standing.md](results/standing.md)（いまの値） |
...
- **サイズ**: 2026-09-24 に S1〜S4 を進めた（推定の損 3 か所は全部取った・旗の全組合せは外れ・
  属性の同時符号化の最初の一手として旗「類」を入れた。`results/losses.md` §34）。残りは、組を 1 記号で送る
  本来の同時符号化・AHN3 と AHN5（幾何v4 が勝つ）に効く案・二次推定（SSE / APM）・KITTI のフレーム間予測
...
進める順の提案は、タイルの確認と速度の小さな直し → 論文（地上型の極座標）→ 残りのサイズの案。
