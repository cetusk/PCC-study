...
（`python/bench_all.py`、ログ `data/work/grid/bench_2m_dir.log`）。
**点の次元・VLR・EVLR・ヘッダの欄まで全部保存したうえでの値**であり、
これが同じものを比べた最初の値である（`results/losses.md`）。
...
幾何だけを取り出すと、**LAZ には 17/17 で勝つ**（比の中央値 0.92x）。
G-PCC には公称値どうしで 11/17。**G-PCC は点の順序を捨てている**ので
そのままでは同じ仕事の比較ではなく、順序の復元も含めて測ると **15/17 ＋ 1 件は決着せず**。
詳細は `results/standing.md` §2.1.1。

時間は LASzip（laspy 既定の lazrs 並列版）に対して次のとおり。

| | 中央値 | 四分位 |
|---|---|---|
| 符号化 | **26.5x**（開始時 99.3x） | [19.9, 30.9] |
| **復号** | **8.3x** | [2.0, 10.0] |

（2026-09-21 時点では 13.6x / 4.3x、サイズは −12.6% だった。
**落としていた次元・VLR・ヘッダを全部保存するようにしたぶん列が増えて遅くなり、
サイズの基準も変わった。**古い値とは直接比べられない。）

**符号化が復号より掛かるのは、候補を全部符号化して最短を採るためである。**
復号は選ばれた 1 本しか通らない。

速度に効いたのは次の 11 で、**最後の 1 つ以外はサイズを 1 bit も変えない**。
...
（上の 9 つは AHN3 _20 100 万点・全 14 列の `enc`。下の 2 つは対 LASzip の比）
