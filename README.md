...
（`python/bench_all.py`、ログ `data/work/grid/bench_2m_v18.log`）。
...
| | 中央値 | 四分位 |
|---|---|---|
| 符号化 | **44.4x**（開始時 99.3x） | [26.2, 53.6] |
| **復号** | **10.1x** | [2.2, 12.9] |

メモリーは 1 点あたり中央値 **739 byte**（属性込み）。三者で最も重い（standing.md §6）。
2026-09-23 の改善ループでサイズを全件で縮めた代わりに、符号化が約 3 割遅くなった
（`results/losses.md` §27〜§32）。
