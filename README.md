...
（`python/bench_all.py`、ログ `data/work/grid/bench_2m_v17.log`）。
...
G-PCC には公称値どうしで 11/17（0.93x）。**G-PCC は点の順序を捨てている**ので
...
| | 中央値 | 四分位 |
|---|---|---|
| 符号化 | **44.6x**（開始時 99.3x） | [29.8, 48.9] |
| **復号** | **9.4x** | [2.3, 12.5] |

メモリーは 1 点あたり中央値 **773 byte**（属性込み）。三者で最も重い（standing.md §6）。
2026-09-23 の改善ループでサイズを全件で縮めた代わりに、符号化が約 3 割遅くなった
（`results/losses.md` §27〜§30）。
