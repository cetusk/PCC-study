...
INPUTS = [
    ...
    ("extrabytes",  "data/raw/extrabytes/extrabytes.las", "las"),
...
def one(item) -> dict:
    ...
    try:
        ...
        out["G-PCC"] = (bs.stat().st_size * 8.0 / m if bs.exists() else float("nan"), r["sec"])
        geom = tmp / "geom.laz"
        M.write_las(geom, xyz, sc, of, None, None)
        out["LAZ"] = (geom.stat().st_size * 8.0 / m, 0.0)
        src = geom
...
def main() -> None:
    ...
    rel = []
    for r in res:
        best = np.nanmin([r[c][0] for c in CODERS[1:]])
        d = 100 * (best / r["G-PCC"][0] - 1)
        rel.append(d)
        wins += d < 0
        print(f"{r['lab']:<13}{r['n']:>8}{r['cont']:>7.2f}"
              ...
              + f"{r['who']:>10}{d:>+8.1f}%")
    a = np.array(rel)
    print(f"\n  系列符号器が勝つ {wins}/{len(res)}   対 G-PCC 中央値 {np.median(a):+.1f}%"
